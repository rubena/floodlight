...
// window.Loadbalancer = Backbone.View.extend({

//     initialize:function () {
//         var self = this;
//         this.template = _.template(tpl.get('loadbalancer'));
...
//             $(this.el).find('table.loadbalancer-table > tbody')
//                 .append("prueba");
//         }, this);
//         return this;
//     }
...
window.LoadbalancerView = Backbone.View.extend({
    initialize:function () {
        this.template = _.template(tpl.get('loadbalancer'));
        this.model.bind("change", this.render, this);
        this.model.bind("add", this.render, this);
    ...
    //render:function (eventName) {
      //  $(this.el).html(this.template());
        ...
        var force = d3.layout.force()
                             ...
                             .linkDistance(200)
                             .size([width, height]);
        var svg = d3.select("#loadbalancer-graph").append("svg")
                    .attr("width", width)
                    .attr("height", height);
        if(this.model.nodes) {
